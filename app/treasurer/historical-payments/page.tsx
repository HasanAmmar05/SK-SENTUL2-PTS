...
export default function HistoricalPaymentsPage() {
  ...
  useEffect(() => {
    const fetchPayments = async () => {
      ...
      try {
        ...
        if (approvedError) {
          console.error(
            "Error fetching approved payments:",
            approvedError.message
          );
        }
        if (rejectedError) {
          console.error(
            "Error fetching rejected payments:",
        ...
        const approvedData = (approved || []).map((p: any) => {
          ...
          if (!p.submitpayment_id) {
            console.warn(
              "Warning: approved payment missing submitpayment_id:",
              p
            );
          }
          return {
            ...
            processed_at:
              p.approved_at || p.created_at || new Date().toISOString(),
          };
        });
        ...
        const rejectedData = (rejected || []).map((p: any) => {
          ...
          if (!p.submitpayment_id) {
            console.warn(
              "Warning: rejected payment missing submitpayment_id:",
              p
            );
          }
          return {
            ...
            processed_at:
              p.rejected_at || p.created_at || new Date().toISOString(),
          };
        });
  ...
  const filteredPayments = useMemo(() => {
    ...
    return payments.filter((payment) => {
      const gradeMatch =
        selectedGrade === "" || payment.grade === selectedGrade;
      const statusMatch =
        filterStatuses.length === 0 || filterStatuses.includes(payment.status);
  ...
  const renderTableRows = () => {
    const paymentsToDisplay = isFullTableDisplayed
      ? filteredPayments
    ...
    if (loading) {
      return (
        <tr>
          <td
            colSpan={5}
            className="px-6 py-8 text-center text-sm text-[var(--text-secondary-historical)]"
          >
            Loading payments...
          </td>
        </tr>
    ...
    <AuthWrapper>
      <div className="relative flex size-full min-h-screen flex-col group/design-root overflow-x-hidden bg-[var(--background-color-historical)]">
        <div className="layout-container flex h-full grow flex-col">
          <MainHeader
            userType="treasurer"
            activePath="/treasurer/historical-payments"
          />
            ...
            <div className="layout-content-container flex flex-col w-full max-w-7xl flex-1">
                ...
                <div className="flex flex-wrap items-center gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    <label className="inline-flex items-center">
                      ...
                      />
                      <span className="ml-2 text-[var(--text-secondary-historical)]">
                        Approved
                      </span>
                    </label>
                    <label className="inline-flex items-center">
              ...
              <div className="bg-[var(--card-background-color-historical)] shadow-xl rounded-xl overflow-hidden @container">
                <div className="overflow-x-auto">
                  <table className="w-full min-w-[800px]" id="paymentTable">
                    ...
                    <tbody
                      className="divide-y divide-[var(--border-color-historical)]"
                      id="paymentTableBody"
                    >
                      {renderTableRows()}
                    </tbody>
              ...
              <div className="mt-8 flex flex-col sm:flex-row items-center justify-between gap-4 px-1">
                <span className="text-sm text-[var(--text-secondary-historical)]">
                  Showing{" "}
                  <span className="font-medium text-[var(--text-primary-historical)]">
                    {displayedCount}
                  </span>{" "}
                  of{" "}
                  <span className="font-medium text-[var(--text-primary-historical)]">
