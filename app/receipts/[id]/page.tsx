  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data: bySubmit } = await supabase
        ...
        .select(
          "submitpayment_id, parent_id, student_name, grade, amount, approved_at, created_at"
        )
      ...
      if (!pay) {
        const { data: byId } = await supabase
          .from("approved_payments")
          .select(
            "id, parent_id, student_name, grade, amount, approved_at, created_at"
          )
          .eq("id", id)
          .single();
      ...
      if (roleCheck.role === "parent") {
        ...
        if (
          ...
          !teacher.assigned_classes.includes(pay.grade)
        ) {
      ...
      const year = (
        pay.approved_at ? new Date(pay.approved_at) : new Date()
      ).getFullYear();
      ...
      const { data: paidData } = await supabase
        .from("approved_payments")
        .select("amount")
        .eq("student_name", pay.student_name)
        .eq("grade", pay.grade)
      ...
      setIssuedDate(
        pay.approved_at
          ? new Date(pay.approved_at).toISOString().split("T")[0]
        ...
        <div className="border-t pt-6 grid grid-cols-2 gap-6">
          <div>
            ...
            <div className="text-base font-semibold">
              MYR {dueAmount.toFixed(2)}
            </div>
          </div>
          <div>
            <div className="text-sm text-slate-600">Balance Remaining</div>
            <div className="text-base font-semibold text-red-600">
              MYR {Math.max(dueAmount - totalPaid, 0).toFixed(2)}
            </div>
          </div>
        </div>
        <div className="mt-8 text-sm text-slate-500">
          Official signature and logo section
        </div>
      </div>
    </div>
  );
